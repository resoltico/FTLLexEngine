...
artifacts to .fuzz_atheris_corpus/roundtrip/findings/ containing the actual FTL
source, S1, S2, and metadata JSON. These artifacts enable debugging without
Atheris and are consumed by fuzz_atheris_replay_finding.py for standalone reproduction.
...
_REPORT_DIR = pathlib.Path(".fuzz_atheris_corpus") / "roundtrip"
...
def _build_stats_dict() -> dict[str, Any]:
    ...
    if _state.iterations > 0:
        stats["junk_ratio"] = round(_domain.junk_skips / _state.iterations, 4)
