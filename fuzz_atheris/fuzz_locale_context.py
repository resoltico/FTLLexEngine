...
- Rounding oracle: formatted digits match Decimal.quantize(ROUND_HALF_EVEN)

Unique coverage (not covered by other fuzzers):
- format_number() direct: min/max fraction digits, grouping, custom pattern
- format_number() numbering_system parameter across CLDR numbering systems
- format_currency() direct: currency precision override, custom pattern
- format_currency() grouping and currency_digits parameter variations
- format_currency() numbering_system parameter
...
- Rounding oracle checks at fuzz scale (ROUND_HALF_EVEN)
...
- format_number_numbering_system: CLDR numbering system parameter
- format_currency_standard: standard currency formatting
- format_currency_display_override: decimal override per currency
- format_currency_custom_pattern: custom Babel currency pattern strings
- format_currency_grouping: use_grouping parameter variations
- format_currency_digits: currency_digits parameter variations
- format_currency_numbering_system: CLDR numbering system for currency
...
- Rounding oracle checks and violations (ROUND_HALF_EVEN)
...
class LocaleContextMetrics:
    ...
    round_half_even_checks: int = 0
    round_half_even_violations: int = 0
...
_PATTERN_WEIGHTS: Sequence[tuple[str, int]] = (
    ...
    ("format_number_numbering_system", 5),
    ("format_currency_standard", 8),
    ("format_currency_display_override", 6),
    ("format_currency_custom_pattern", 5),
    ("format_currency_grouping", 6),
    ("format_currency_digits", 6),
    ("format_currency_numbering_system", 5),
...
# CLDR numbering system identifiers supported by Babel
_NUMBERING_SYSTEMS: Sequence[str] = ("latn", "arab", "arabext", "deva", "beng")
...
def _build_stats_dict() -> FuzzStats:
    ...
    stats["round_half_even_checks"] = _domain.round_half_even_checks
    stats["round_half_even_violations"] = _domain.round_half_even_violations
...
def _check_rounding(
    ...
    """Oracle: formatted digits must match ROUND_HALF_EVEN quantization.

    Babel uses ROUND_HALF_EVEN (banker's rounding) as its default rounding mode.
    Normalizes locale-specific decimal/group symbols to ASCII before digit
    extraction, then compares the numeric part against
    ...
    _domain.round_half_even_checks += 1
    try:
    ...
    if actual != expected:
        _domain.round_half_even_violations += 1
        _state.findings += 1
        ctx = f"currency={currency}" if currency else "number"
        msg = (
            f"rounding violation ({ctx}): "
...
def _pattern_format_number_int(fdp: atheris.FuzzedDataProvider) -> None:
    """Integer values formatted across locales."""
...
def _pattern_format_number_decimal(fdp: atheris.FuzzedDataProvider) -> None:
    """Decimal/float values with precision formatted across locales."""
    locale_code = fdp.PickValueInList(list(_VALID_LOCALES))
    ctx = LocaleContext.create(locale_code)
    _domain.format_number_calls += 1
...
def _pattern_format_number_precision(fdp: atheris.FuzzedDataProvider) -> None:
    """Min/max fraction digit combinations with rounding oracle."""
    locale_code = fdp.PickValueInList(list(_VALID_LOCALES))
    ctx = LocaleContext.create(locale_code)
    _domain.format_number_calls += 1
    int_part = fdp.ConsumeIntInRange(-9999, 9999)
    ...
    _check_rounding(result, value, max_frac, locale_code)


def _pattern_format_number_custom_pattern(fdp: atheris.FuzzedDataProvider) -> None:
    """Custom Babel number pattern strings."""
    locale_code = fdp.PickValueInList(list(_VALID_LOCALES))
    ctx = LocaleContext.create(locale_code)
    _domain.format_number_calls += 1
    int_part = fdp.ConsumeIntInRange(-9999, 9999)
...
def _pattern_format_number_grouping(fdp: atheris.FuzzedDataProvider) -> None:
    """grouping=True/False variations across large values."""
    locale_code = fdp.PickValueInList(list(_VALID_LOCALES))
    ctx = LocaleContext.create(locale_code)
    _domain.format_number_calls += 1
    value = Decimal(
...
def _pattern_format_number_numbering_system(fdp: atheris.FuzzedDataProvider) -> None:
    """CLDR numbering system parameter variations for format_number."""
    locale_code = fdp.PickValueInList(list(_VALID_LOCALES))
    ctx = LocaleContext.create(locale_code)
    _domain.format_number_calls += 1
    int_part = fdp.ConsumeIntInRange(-9999, 9999)
    frac_part = fdp.ConsumeIntInRange(0, 99)
    value = Decimal(f"{int_part}.{frac_part:02d}")
    ns = fdp.PickValueInList(list(_NUMBERING_SYSTEMS))
    result = ctx.format_number(value, numbering_system=ns)
    if not result:
        msg = (
            f"format_number({value}, numbering_system='{ns}') "
            f"empty for {locale_code}"
        )
        raise LocaleContextFuzzError(msg)
    # Determinism: same call twice must return same result
    result2 = ctx.format_number(value, numbering_system=ns)
    if result != result2:
        msg = (
            f"Determinism violation: format_number({value}, ns='{ns}') "
            f"for '{locale_code}': '{result}' != '{result2}'"
        )
        raise LocaleContextFuzzError(msg)


def _pattern_format_currency_standard(fdp: atheris.FuzzedDataProvider) -> None:
    """Standard currency formatting with rounding oracle."""
    locale_code = fdp.PickValueInList(list(_VALID_LOCALES))
    ctx = LocaleContext.create(locale_code)
    ...
    if not result:
        ...
        raise LocaleContextFuzzError(msg)
    # Oracle: precision=None lets _check_rounding derive the
    # CLDR-standard decimal count per currency (KWD=3, JPY=0, USD=2).
    _check_rounding(result, value, None, locale_code, currency=currency)


def _pattern_format_currency_display_override(
    fdp: atheris.FuzzedDataProvider,
) -> None:
    """Currency display override (symbol, code, name)."""
    locale_code = fdp.PickValueInList(list(_VALID_LOCALES))
    ctx = LocaleContext.create(locale_code)
    _domain.format_currency_calls += 1
    currency = fdp.PickValueInList(list(_CURRENCIES))
...
def _pattern_format_currency_custom_pattern(
    fdp: atheris.FuzzedDataProvider,
) -> None:
    """Custom Babel currency pattern strings."""
    locale_code = fdp.PickValueInList(list(_VALID_LOCALES))
    ctx = LocaleContext.create(locale_code)
    _domain.format_currency_calls += 1
...
def _pattern_format_currency_grouping(fdp: atheris.FuzzedDataProvider) -> None:
    """use_grouping parameter variations for format_currency."""
    locale_code = fdp.PickValueInList(list(_VALID_LOCALES))
    ctx = LocaleContext.create(locale_code)
    _domain.format_currency_calls += 1
    currency = fdp.PickValueInList(list(_CURRENCIES))
    int_part = fdp.ConsumeIntInRange(0, 999_999)
    frac_part = fdp.ConsumeIntInRange(0, 99)
    value = Decimal(f"{int_part}.{frac_part:02d}")
    use_grouping = fdp.ConsumeBool()
    result = ctx.format_currency(value, currency=currency, use_grouping=use_grouping)
    if not result:
        msg = (
            f"format_currency({value}, currency={currency}, "
            f"use_grouping={use_grouping}) empty for {locale_code}"
        )
        raise LocaleContextFuzzError(msg)


def _pattern_format_currency_digits(fdp: atheris.FuzzedDataProvider) -> None:
    """currency_digits parameter variations for format_currency."""
    locale_code = fdp.PickValueInList(list(_VALID_LOCALES))
    ctx = LocaleContext.create(locale_code)
    _domain.format_currency_calls += 1
    currency = fdp.PickValueInList(list(_CURRENCIES))
    ...
    result = ctx.format_currency(
        value, currency=currency, currency_digits=currency_digits
    )
    if not result:
        msg = (
            f"format_currency({value}, currency={currency}, "
            f"currency_digits={currency_digits}) empty for {locale_code}"
        )
        raise LocaleContextFuzzError(msg)


def _pattern_format_currency_numbering_system(
    fdp: atheris.FuzzedDataProvider,
) -> None:
    """CLDR numbering system parameter for format_currency."""
    locale_code = fdp.PickValueInList(list(_VALID_LOCALES))
    ctx = LocaleContext.create(locale_code)
    _domain.format_currency_calls += 1
    currency = fdp.PickValueInList(list(_CURRENCIES))
    ...
    value = Decimal(f"{int_part}.{frac_part:02d}")
    ns = fdp.PickValueInList(list(_NUMBERING_SYSTEMS))
    result = ctx.format_currency(value, currency=currency, numbering_system=ns)
    if not result:
        msg = (
            f"format_currency({value}, currency={currency}, "
            f"numbering_system='{ns}') empty for {locale_code}"
        )
        raise LocaleContextFuzzError(msg)


def _pattern_format_datetime_date_obj(fdp: atheris.FuzzedDataProvider) -> None:
    """Plain date objects promoted to midnight datetime."""
    locale_code = fdp.PickValueInList(list(_VALID_LOCALES))
    ctx = LocaleContext.create(locale_code)
...
def _pattern_format_datetime_datetime_obj(fdp: atheris.FuzzedDataProvider) -> None:
    """datetime objects with date styles, tz-aware and naive variants."""
    locale_code = fdp.PickValueInList(list(_VALID_LOCALES))
    ctx = LocaleContext.create(locale_code)
    _domain.format_datetime_calls += 1
    year = fdp.ConsumeIntInRange(2000, 2030)
...
def _pattern_format_datetime_style_combo(fdp: atheris.FuzzedDataProvider) -> None:
    """date_style / time_style combinations."""
    locale_code = fdp.PickValueInList(list(_VALID_LOCALES))
    ctx = LocaleContext.create(locale_code)
    _domain.format_datetime_calls += 1
    year = fdp.ConsumeIntInRange(2000, 2030)
...
def _pattern_format_datetime_pattern(fdp: atheris.FuzzedDataProvider) -> None:
    """Custom Babel datetime pattern strings."""
    locale_code = fdp.PickValueInList(list(_VALID_LOCALES))
    ctx = LocaleContext.create(locale_code)
    _domain.format_datetime_calls += 1
    year = fdp.ConsumeIntInRange(2000, 2030)
...
def _pattern_locale_create_adversarial(fdp: atheris.FuzzedDataProvider) -> None:
    """LocaleContext.create() with fuzz-generated locale codes."""
...
def _pattern_cross_locale_determinism(fdp: atheris.FuzzedDataProvider) -> None:
    """Same value formatted by N locales; same locale must yield same result."""
    locale_code = fdp.PickValueInList(list(_VALID_LOCALES))
...
_PATTERN_DISPATCH = {
    "format_number_int": _pattern_format_number_int,
    "format_number_decimal": _pattern_format_number_decimal,
    "format_number_precision": _pattern_format_number_precision,
    "format_number_custom_pattern": _pattern_format_number_custom_pattern,
    "format_number_grouping": _pattern_format_number_grouping,
    "format_number_numbering_system": _pattern_format_number_numbering_system,
    "format_currency_standard": _pattern_format_currency_standard,
    "format_currency_display_override": _pattern_format_currency_display_override,
    "format_currency_custom_pattern": _pattern_format_currency_custom_pattern,
    "format_currency_grouping": _pattern_format_currency_grouping,
    "format_currency_digits": _pattern_format_currency_digits,
    "format_currency_numbering_system": _pattern_format_currency_numbering_system,
    "format_datetime_date_obj": _pattern_format_datetime_date_obj,
    "format_datetime_datetime_obj": _pattern_format_datetime_datetime_obj,
    "format_datetime_style_combo": _pattern_format_datetime_style_combo,
    "format_datetime_pattern": _pattern_format_datetime_pattern,
    "locale_create_adversarial": _pattern_locale_create_adversarial,
    "cross_locale_determinism": _pattern_cross_locale_determinism,
}
...
def test_one_input(data: bytes) -> None:
    ...
    try:
        ...
        is_interesting = (
            ...
            or pattern_name in (
                "locale_create_adversarial",
                "cross_locale_determinism",
                "format_number_numbering_system",
                "format_currency_numbering_system",
            )
