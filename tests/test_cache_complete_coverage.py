...
from ftllexengine.diagnostics import FluentError, FluentReferenceError
from ftllexengine.runtime.cache import FormatCache


class TestFormatCacheInitValidation:
    """Test FormatCache.__init__ parameter validation."""

    def test_maxsize_zero_rejected(self) -> None:
        """FormatCache rejects maxsize=0."""
        with pytest.raises(ValueError, match="maxsize must be positive"):
            FormatCache(maxsize=0)

    def test_maxsize_negative_rejected(self) -> None:
        """FormatCache rejects negative maxsize."""
        with pytest.raises(ValueError, match="maxsize must be positive"):
    ...
    def test_max_errors_per_entry_zero_rejected(self) -> None:
        ...
        with pytest.raises(ValueError, match="max_errors_per_entry must be positive"):
            FormatCache(max_errors_per_entry=0)

    def test_max_errors_per_entry_negative_rejected(self) -> None:
        """FormatCache rejects negative max_errors_per_entry."""
        with pytest.raises(ValueError, match="max_errors_per_entry must be positive"):
            FormatCache(max_errors_per_entry=-1)


class TestFormatCacheUnhashableHandling:
    """Test FormatCache handling of unhashable arguments."""
...
class TestFormatCacheErrorBloatProtection:
    """Test FormatCache error collection memory bounding."""

    def test_put_rejects_excessive_error_count(self) -> None:
        """put() skips caching when error count exceeds max_errors_per_entry."""
        cache = FormatCache(max_errors_per_entry=10)
...
class TestFormatCacheLRUBehavior:
    """Test FormatCache LRU eviction and move-to-end behavior."""
...
class TestFormatCacheMakeHashableDepth:
    """Test FormatCache._make_hashable depth limit enforcement."""
    ...
    def test_make_hashable_depth_limit_list(self) -> None:
        ...
        with pytest.raises(TypeError, match="Maximum nesting depth exceeded"):
            FormatCache._make_hashable(deep_list)
    ...
    def test_make_hashable_depth_limit_dict(self) -> None:
        ...
        with pytest.raises(TypeError, match="Maximum nesting depth exceeded"):
            FormatCache._make_hashable(deep_dict)
    ...
    def test_make_hashable_depth_limit_mixed(self) -> None:
        ...
        with pytest.raises(TypeError, match="Maximum nesting depth exceeded"):
            FormatCache._make_hashable(deep_structure)


class TestFormatCacheMakeHashableTypes:
    """Test FormatCache._make_hashable type conversion."""

    def test_make_hashable_primitives(self) -> None:
        """_make_hashable preserves primitives unchanged."""
        assert FormatCache._make_hashable("text") == "text"
        assert FormatCache._make_hashable(42) == 42
        assert FormatCache._make_hashable(3.14) == 3.14
        assert FormatCache._make_hashable(True) is True
        assert FormatCache._make_hashable(None) is None

    def test_make_hashable_decimal(self) -> None:
        """_make_hashable preserves Decimal unchanged."""
        decimal_value = Decimal("123.45")
        result = FormatCache._make_hashable(decimal_value)
    ...
    def test_make_hashable_datetime(self) -> None:
        ...
        result = FormatCache._make_hashable(dt)
        assert result == dt
    ...
    def test_make_hashable_date(self) -> None:
        ...
        d = date(2024, 1, 1)
        result = FormatCache._make_hashable(d)
        assert result == d
    ...
    def test_make_hashable_unknown_type_raises(self) -> None:
        ...
        with pytest.raises(TypeError, match="Unknown type in cache key"):
            FormatCache._make_hashable(CustomType())


class TestFormatCacheMakeKeyRobustness:
    """Test FormatCache._make_key error handling."""
...
class TestFormatCacheProperties:
    """Test FormatCache property accessors."""
...
class TestFormatCacheHypothesisProperties:
    """Property-based tests for FormatCache using Hypothesis."""
