...
class TestParseDateHypothesis:
    ...
    @given(
        year=st.integers(min_value=2000, max_value=2068),  # 2-digit year limit for CLDR
...
class TestDateParsingEdgeCases:
    ...
    def test_parse_date_locale_missing_date_formats(
        ...
        # Non-ISO format that won't match en_US CLDR patterns
        # (European dd.MM.yyyy — dot separator not used by en_US)
        result, _errors = parse_date("28.01.2025", "en_US")
        assert result is None
    ...
    def test_parse_datetime_locale_missing_datetime_formats(
        ...
        # No fallback patterns - should return None with error
...
class TestFourDigitYearProperty:
    """Property tests: 4-digit year inputs accepted for locales with yy CLDR short format.

    Verifies that the %Y variant generated alongside %y patterns accepts 4-digit
    year strings and returns the same date as the canonical 2-digit form.
    """

    _LOCALES_WITH_YY_SHORT: tuple[str, ...] = ("lv_LV", "de_DE", "pl_PL", "fi_FI", "ru_RU")

    @given(
        year=st.integers(min_value=2000, max_value=2068),
        month=st.integers(min_value=1, max_value=12),
        day=st.integers(min_value=1, max_value=28),
        locale=st.sampled_from(_LOCALES_WITH_YY_SHORT),
    )
    @settings(max_examples=200)
    def test_four_digit_year_parses_same_date_as_two_digit(
        self, year: int, month: int, day: int, locale: str
    ) -> None:
        """parse_date with 4-digit year returns the same date as ISO 8601 for the locale.

        For locales where the CLDR short format uses a 2-digit year (dd.MM.yy),
        supplying a 4-digit year in the same positional format must yield the
        correct date, not a parse failure.
        """
        event(f"locale={locale}")
        event(f"year_range={'2000s' if year <= 2050 else 'late_2050s'}")

        # ISO 8601 is the ground-truth reference
        iso_str = f"{year:04d}-{month:02d}-{day:02d}"
        expected_date = date(year, month, day)

        iso_result, iso_errors = parse_date(iso_str, locale)
        assert not iso_errors
        assert iso_result == expected_date

        # Now try parsing a 4-digit dotted form (common European format)
        # This tests the %Y variant generated by _extract_cldr_patterns.
        four_digit_str = f"{day:02d}.{month:02d}.{year:04d}"
        four_result, four_errors = parse_date(four_digit_str, locale)

        if four_result is not None:
            event("outcome=four_digit_parsed")
            assert four_result == expected_date, (
                f"4-digit parse of {four_digit_str!r} ({locale}) returned "
                f"{four_result}, expected {expected_date}"
            )
        else:
            # Locale may not use a dotted format — acceptable if errors are present
            event("outcome=four_digit_no_match")
            assert four_errors, (
                f"parse_date({four_digit_str!r}, {locale!r}) returned None with no errors"
            )
    ...
    @settings(max_examples=200)
    def test_lv_lv_four_digit_year_always_parses(
        self, year: int, month: int, day: int
    ) -> None:
        """lv-LV dd.MM.yyyy form always parses to the correct date.

        lv-LV CLDR short is dd.MM.yy. The generated %Y variant must accept
        all 4-digit years in the same positional format.
        """
        event(f"year={year}")
        four_digit_str = f"{day:02d}.{month:02d}.{year:04d}"
        result, errors = parse_date(four_digit_str, "lv_LV")
        assert not errors, f"parse_date({four_digit_str!r}, lv_LV) failed: {errors}"
