...
class TestRWLockDecoratorErrorHandling:
    """Test decorator error handling."""

    def test_read_decorator_missing_lock_attribute(self) -> None:
        """with_read_lock raises AttributeError if lock attribute missing."""
    ...
    def test_write_decorator_missing_lock_attribute(self) -> None:
        """with_write_lock raises AttributeError if lock attribute missing."""

        class NoLock:
            @with_write_lock()
            def write_method(self) -> None:
                pass
    ...
    def test_read_decorator_custom_attr_missing(self) -> None:
        """with_read_lock with custom attr raises if attr missing."""

        class NoLock:
            def __init__(self) -> None:
                self._rwlock = RWLock()  # Wrong name

            @with_read_lock(lock_attr="custom_lock")
            def read_method(self) -> None:
                pass
    ...
    def test_write_decorator_custom_attr_missing(self) -> None:
        """with_write_lock with custom attr raises if attr missing."""

        class NoLock:
            def __init__(self) -> None:
                self._rwlock = RWLock()  # Wrong name

            @with_write_lock(lock_attr="custom_lock")
            def write_method(self) -> None:
                pass
    ...
    def test_read_decorator_wrong_lock_type(self) -> None:
        """with_read_lock raises if attribute is not RWLock."""

        class WrongLockType:
            def __init__(self) -> None:
                self._rwlock = "not a lock"  # Wrong type
    ...
    def test_write_decorator_wrong_lock_type(self) -> None:
        """with_write_lock raises if attribute is not RWLock."""
    ...
    def test_decorator_exception_propagates(self) -> None:
        """Exceptions from decorated methods propagate correctly."""
        ...
        class Container:
            ...
            @with_read_lock()
            def read_raises(self) -> None:
                msg = "Read error"
                raise ValueError(msg)

            @with_write_lock()
            def write_raises(self) -> None:
                msg = "Write error"
    ...
    def test_decorator_locks_released_after_exception(self) -> None:
        """Locks released even when decorated method raises exception."""
        ...
        # Read decorator releases lock on exception
        ...
        # Write decorator releases lock on exception
        with pytest.raises(ValueError, match="Error"):
            obj.write_raises()

        # Should be able to acquire write lock again
        obj.write_value(99)
        assert obj.value == 99
