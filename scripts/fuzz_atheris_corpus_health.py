#!/usr/bin/env python3
"""Atheris Corpus Health Checker.

Validates the Atheris seed corpus (fuzz_atheris/seeds/), computes FTL grammar
coverage metrics, and reports issues like duplicates or parse errors.

NOTE: This script is for ATHERIS corpus only. Hypothesis/HypoFuzz uses
.hypothesis/examples/ which is managed automatically by the framework.

Usage:
    uv run python scripts/fuzz_atheris_corpus_health.py
    uv run python scripts/fuzz_atheris_corpus_health.py --json
    uv run python scripts/fuzz_atheris_corpus_health.py --dedupe
    uv run python scripts/fuzz_atheris_corpus_health.py --coverage
"""
...
SEEDS_DIR = Path(__file__).parent.parent / "fuzz_atheris" / "seeds"
